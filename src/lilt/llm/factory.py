...
class ProviderFactory:
    """Instantiates the correct LLMProvider based on the configuration."""

    @staticmethod
    def create(config: dict[str, Any]) -> LLMProvider:
        """Create an LLM provider based on the llm config block.

        If 'stages' is present, builds a RouterLLMProvider mapping each stage to its provider.
        """
    ...
    def _create_single(config: dict[str, Any]) -> LLMProvider:
        ...
        if provider_name == "openai":
            ...
            raise ValueError(
                f"Unsupported LLM provider: {provider_name}. Supported providers: openai"
            )
