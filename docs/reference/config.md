...
`lilt project init` writes a **short required block** plus commented advanced keys.
Schema defaults still apply for omitted keys (`LiltConfig`). Full catalog below.

Minimal shape written by init:
...
parser:
  custom_macros: []
  block_transparent_macros: [section, subsection, …]
```

Authoritative field catalog (including advanced keys):
