...
Interrupted runs: **re-invoke** `translate` (no separate resume command). Finished segments stay in the TM. `Ctrl+C` / SIGTERM aborts **between segments** (exit 130); the current segment’s LLM call may still finish. If conflicts/errors remain after an idle run, the CLI exits non-zero, names the affected namespace(s), and points at `tm list <ns> --status conflict` / `build --allow-partial`. Progress Live is **transient** while work runs (current stage / segment). Durable output uses the CLI glyph language (`✓` / `!` / `i`): in **workflow** mode each reflection stage that ran prints `✓ ns — draft|critique|refine done (N segments)` with a **per-stage** count (not cumulative across D→C→R); sequential / single-batch runs print `✓ ns — Done (N segments)`. Boring idle (`already translated` / empty) is collapsed into one `i Skipped …` summary; conflict idle prints a short `! ns — N conflict/error` before the final hint. Empty post-sync namespaces (0 segments, e.g. TikZ-only includes) are idle-skipped, including under `--all`.
